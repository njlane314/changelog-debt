...
export function loadConfig(configPath = DEFAULT_CONFIG_PATH, cwd = process.cwd(), overrides: Partial<ChangelogDebtConfig> = {}): ChangelogDebtConfig {
  const resolved = path.resolve(cwd, configPath);
  let section: unknown = {};
  if (fs.existsSync(resolved)) {
    const parsed = parse(fs.readFileSync(resolved, 'utf8')) ?? {};
    section = typeof parsed === 'object' && parsed !== null && 'changelog_debt' in parsed ? (parsed as { changelog_debt?: unknown }).changelog_debt ?? {} : parsed;
  }
